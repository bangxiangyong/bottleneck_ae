...
decoder_mu = infer_decoder(
    encoder, activation=mid_activation, last_activation="sigmoid"
...
run_auto_lr_range_v2(
    train_loader, bae_ensemble, run_full=True, window_size=2, num_epochs=10
)
...
raw_train_scores_perc = convert_percentile(
    raw_train_scores, raw_train_scores
)  # convert to pct

# get threshold
threshold_perc = 0.10
anomaly_threshold = stats.scoreatpercentile(
    raw_train_scores_perc, 100 - (100 * threshold_perc)
)
...
y_pred_grid = (
    bae_ensemble.predict_samples(scaler.transform(grid_2d), select_keys=["se"])
    .mean(0)[0]
    .mean(-1)
)
...
plot_decision_boundary(
    x_inliers_train=x_inliers_train,
    x_inliers_test=x_inliers_test,
    x_outliers_test=x_outliers_test[random_anomaly_index],
    grid_2d=grid_2d,
    Z=y_pred_grid,
    # anomaly_threshold=anomaly_threshold,
)
