...
for dataset in paths.keys():
    ...
    optim_df = (
        apply_optim_df(
            raw_df,
            fixed_params=["BTNECK_TYPE+INF", "bae_type"],
            optim_params=[
                "current_epoch",
                "latent_factor",
                "skip",
...
for dataset_i, dataset_targetdim in enumerate(datasets):
    ...
    # YTICKS AT 5 LEVELS
    N = 5
    n_deci = 3 if dataset_targetdim == "ZEMA-0" else 2
    ymin, ymax = ax.get_ylim()
    ymax = np.clip(ymax, None, 1)
    # ax.set_yticks(np.clip(np.round(np.linspace(ymin, ymax, N), n_deci), None, 1))
    ax.set_yticks(np.round(np.linspace(ymin, ymax, N), n_deci))
