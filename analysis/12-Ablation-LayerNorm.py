...
for dataset in paths.keys():
    ...
    fixed_inf_params = ["bae_type", "layer_norm"]
    optim_inf_params = [
        "W_std",
        "diag_reg",
        "norm",
    ...
    optim_df_combined_ = optim_inf_df.copy()
...
layer_labels = {"layer": "+norm", "none": "\N{MINUS SIGN}norm"}
...
for i, dataset_targetdim in enumerate(datasets):
    ...
    temp_df = temp_df.replace(list(layer_labels.keys()), list(layer_labels.values()))

    temp_df = temp_df.replace(
        list(bottleneck_map.keys()), list(bottleneck_map.values())
    )[["bae_type", "layer_norm", metric_col]]
    # select important columns only

    # fmt: off
    pivot_df_mean = temp_df.groupby(["bae_type", "layer_norm"]).mean().reset_index()[["bae_type", "layer_norm", metric_col]].pivot(index="bae_type", columns="layer_norm", values=metric_col).reset_index()
    pivot_df_mean = temp_df.groupby(["bae_type", "layer_norm"]).mean().reset_index()[["bae_type", "layer_norm", metric_col]].copy()
    # fmt: on

    # HORIZONTAL
    ax.ticklabel_format(axis="y", scilimits=(-3, 0), useMathText=True)
    bplot = sns.barplot(
        x="layer_norm",
        # hue="BTNECK_TYPE",
        y=metric_col,
        ...
        order=list(layer_labels.values()),
        # zorder=3,
        # color="tab:blue",
    )

    xmax = temp_df[metric_col].max()
    xmin = temp_df[metric_col].min()

    # if -xmin > xmax:
    #     xmin = -xmax
    dist = (xmax - xmin) * 0.15
    ax.set_ylim(xmin - dist, xmax + dist)
...
#     else:
#         offset2 = 2
#     # ax.set_yticks(np.round(np.linspace(xmin, xmax, N), offset2))
#     ax.set_yticks(np.clip(np.round(np.linspace(xmin, xmax, N), offset2), None, 1.0))
#
#     ax.set_ylabel("AUROC (" + r"$\times10^{-" + str(offset2) + r"})$", fontsize="small")
#
...
#     ax.set_yticks(new_xticks)
#     ax.set_yticklabels(new_xticklabels)
...
if save_fig:
    filename = "plots/layernorm-" + str(dataset_subsets_i) + ".png"
