...
# raw_df = raw_df[raw_df["n_conv_layers"] == 5]
...
optim_df = apply_optim_df(
    ...
    optim_params=["current_epoch", "layer_norm", "n_conv_layers"],
    perf_key=metric_col,
    target_dim_col=tasks_col_map[dataset],
)
...
for target_dim in optim_df[tasks_col_map[dataset]].unique():
    ...
    temp_df_AB = (
        temp_df_AB.groupby(["bae_type", "latent_factor"])
        .mean()[metric_col]
        .reset_index()
    )
    temp_df_CD = (
        temp_df_CD.groupby(["bae_type", "latent_factor"])
        .mean()[metric_col]
        .reset_index()
    )
