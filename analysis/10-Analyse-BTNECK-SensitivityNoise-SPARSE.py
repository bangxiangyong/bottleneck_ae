...
def calc_srm(df_inf_mean_):
    ...
    """
    mean_change = np.nanmean(df_inf_mean_["E_AUROC"][1:]) * 100

    return mean_change


temp_inf = (
    inf_df.groupby(["noise_type", "W_std", "num_layers", "noise_scale", tasks_col_name])
    .mean()
    .reset_index()
)
# "noise_scale",
groupcols = ["noise_type", "W_std", "num_layers", tasks_col_name]
all_models = temp_inf[groupcols].drop_duplicates()

for _, row in all_models.iterrows():
    len_rows = len(temp_inf.merge(pd.DataFrame(row).T))
    print(len_rows)
...
for target_dim in optim_inf_df[tasks_col_name].unique():
    for noise_type in ["normal", "uniform"]:
        ...
        # ==========FINITE: NO BTNECK SAE====================
        optim_df_sae = apply_optim_df(
            raw_df[raw_df["weight_decay"] == 0],
            fixed_params=["bae_type", "HAS_BTNECK", "noise_type", "noise_scale"],
            optim_params=["current_epoch", "layer_norm"],
            perf_key="E_AUROC",
            target_dim_col=tasks_col_map[dataset],
...
# df_mean, df_sem = get_mean_sem(
#     temp_inf_df,
#     conditions={tasks_col_name: target_dim, "noise_type": noise_type},
#     groupby=["noise_scale", "layer_norm"],
# )
#
#
# plt.figure()
# plt.plot(df_mean["noise_scale"], df_mean["E_AUROC"])
