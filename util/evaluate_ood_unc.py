...
def evaluate_ood_unc(
    ...
    # aggregation depends on whether eval_ood_unc is turned on
    if eval_ood_unc:
        aggregate = False
    else:
        aggregate = True
    ...
    bae_id_pred = bae_model.predict(
        x_id_test, select_keys=[nll_key, "y_mu"], aggregate=aggregate
    )
    bae_ood_pred = bae_model.predict(
        x_ood_test, select_keys=[nll_key, "y_mu"], aggregate=aggregate
    )
    # get ood scores
    if aggregate:
        e_nll_id = bae_id_pred[nll_key + "_mean"].mean(-1)
        e_nll_ood = bae_ood_pred[nll_key + "_mean"].mean(-1)
        var_nll_id = bae_id_pred[nll_key + "_var"].mean(-1)
        var_nll_ood = bae_ood_pred[nll_key + "_var"].mean(-1)
        var_x_id = bae_id_pred["y_mu" + "_var"].mean(-1)
        var_x_ood = bae_ood_pred["y_mu" + "_var"].mean(-1)

        waic_id = bae_id_pred["waic"].mean(-1)
        waic_ood = bae_ood_pred["waic"].mean(-1)
    else:
        ...
        waic_id = e_nll_id + var_nll_id
        waic_ood = e_nll_ood + var_nll_ood

    eval_auroc = {
        "E_AUROC": calc_auroc(e_nll_id, e_nll_ood),
        "V_AUROC": calc_auroc(var_nll_id, var_nll_ood),
        "WAIC_AUROC": calc_auroc(waic_id, waic_ood),
        "VX_AUROC": calc_auroc(var_x_id, var_x_ood),
    }

    eval_avgprc = {
        "E_AVGPRC": calc_avgprc(e_nll_id, e_nll_ood),
        "V_AVGPRC": calc_avgprc(var_nll_id, var_nll_ood),
        "WAIC_AVGPRC": calc_avgprc(waic_id, waic_ood),
        "VX_AVGPRC": calc_avgprc(var_x_id, var_x_ood),
    }

    if exp_man is None:
        exp_man = ExperimentManager()
    exp_man.update_csv(
        exp_params=exp_man.concat_params_res(exp_params, eval_auroc),
        csv_name=exp_name + "AUROC.csv",
    )
    exp_man.update_csv(
        exp_params=exp_man.concat_params_res(exp_params, eval_avgprc),
        csv_name=exp_name + "AVGPRC.csv",
    )
    ...
    if eval_bce_se:
        if (
            ...
            eval_auroc_bce_se = evaluate_bce_se(
                bae_model, x_id_test, x_ood_test, aggregate=aggregate
            )
            if exp_man is None:
                exp_man = ExperimentManager()
    ...
    if eval_ood_unc:
        ...
        bae_id_ref_pred = bae_model.predict(
            x_id_train, select_keys=[nll_key], aggregate=aggregate
        )
    ...
    if ret_flatten_nll and eval_ood_unc:
        ...
        return (bae_id_pred, bae_ood_pred), (eval_auroc, eval_avgprc, {}, {})
